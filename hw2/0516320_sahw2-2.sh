...
dialog_CPUUsage() {
    hw_ncpu=$(sysctl hw.ncpu)
    hw_ncpu=${hw_ncpu#"hw.ncpu: "}
    if [ $hw_ncpu -eq 1 ]; then
        Usage=0
        while [ $Usage -le 100 ]; do
        USER=$(top | grep CPU | sed 's/^.*CPU://g' | sed 's/%.*$//' | grep "[0-9]")
        SYST=$(top | grep CPU | sed 's/^.*nice,//g'| sed 's/%.*$//' | grep "[0-9]")
        IDLE=$(top | grep CPU | sed 's/^.*rupt,//g'| sed 's/%.*$//' | grep "[0-9]")
        CPUSGresult=" CPU LOADING\n\n CPU0:  USER: ${USER}%  SYST: ${SYST}%  IDLE: ${IDLE}%"
        echo $Usage|dialog --backtitle "$BACKTITLE" --title "CPU Usage" --gauge "$CPUSGresult" 40 90
        read -t 1 key
        if [ $? -eq 0 ]; then
            break
        fi
